...
FROM(
        ...
        WHERE (0=0)
          ...
          AND NFCAB.NOTACONF IN (:CONFNOTA)
        ...
        SELECT ITEMAGRO.ITEM,
               VPEDITEM.NUMERO DOCUMENTO,
               ITEMAGRO.DESCRICAO,
               VPEDITEM.SALDO_QTD
        FROM VPEDITEM
        INNER JOIN PEDCAB
        ON PEDCAB.ESTAB = VPEDITEM.ESTAB
        AND PEDCAB.SERIE = VPEDITEM.SERIE
        AND PEDCAB.NUMERO = VPEDITEM.NUMERO
        INNER JOIN ITEMAGRO
        ON ITEMAGRO.ITEM = VPEDITEM.ITEM
        INNER JOIN CONTAMOV
        ON CONTAMOV.NUMEROCM = PEDCAB.PESSOA
        WHERE VPEDITEM.SALDO_QTD > 0
        AND CONTAMOV.CNPJF = :CPF
        AND PEDCAB.PEDIDOCONF IN (:CONFPEDIDO)
